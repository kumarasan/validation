...
export default class Validator {
  ...
  constructor(options = {}) {
    ...
    this.rules = {
      ...
      phone: { message: 'The :attribute must be a valid phone number.', rule: val => this.helpers.testRegex(val, /^[0-9]{6,10}$/)},
      ...
      list_name: { message: 'The :attribute is not a valid.', rule: val => this.helpers.testRegex(val, /^[^:?*#{}(\/\\)%^&]+$/), required: true },
      no_space: { message: 'Space is not allowed.', rule: val => this.helpers.testRegex(val, /^[^\s].+[^\s]$/), required: true },
      ...(options.validators || {}),
    };
  ...
  message(field, inputValue, validations, options = {}) {  
    ...
    if (Array.isArray(inputValue)) {
      let message = inputValue.length === 0 && this.helpers.message(validations, field, options, validations);
      if (this.visibleFields.includes(field) && inputValue.length === 0) {
